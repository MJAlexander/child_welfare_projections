    ...
    tabItems(
      ...
      tabItem(
        ...
        sidebarLayout(
          sidebarPanel(
            ...
            selectInput(
              inputId = "indicator", label = "Indicator",
              c("Entries", "Permanent exits", "Non-permanent exits", "Investigations")
            ),
            selectInput(
              inputId = "race", label = "Race/ethnicity",
      ...
      tabItem(
        ...
        sidebarLayout(
          sidebarPanel(
            ...
            selectInput(
              inputId = "indicator_2", label = "Indicator",
              c("Entries", "Permanent exits", "Non-permanent exits", "Investigations")
            ),
            selectInput(
              inputId = "race_2", label = "Race/ethnicity",
              c("Total", "Non-Hispanic White", "Non-Hispanic Black", "Non-Hispanic Asian/Pacific Islander", "Non-Hispanic American Indian/Alaska Native", "Hispanic")
            ),
            radioButtons(
              inputId = "type_2",
      ...
      tabItem(
        ...
        sidebarLayout(
          sidebarPanel(
            selectInput(
              inputId = "indicator_3", label = "Indicator",
              c("Entries", "Permanent exits", "Non-permanent exits", "Investigations")
            ),
...
server <- function(input, output) {
  ...
  output$TimePlot <- renderPlotly({
    if (input$type_2 == "per capita") {
      ...
      p <- ggplot(
        data = d_res %>% filter(state == input$state, indicator == input$indicator_2, race == input$race_2, ci_width == 0.75),
  ...
  output$CoefTable <- renderTable(betas %>%
    filter(indicator == input$indicator_2, race == input$race_2, division == state_div %>%
